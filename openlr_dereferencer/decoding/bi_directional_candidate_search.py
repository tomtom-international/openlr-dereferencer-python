...
def bi_directional_search(
    ...
    forwd_lrp, backwd_lrp = lrps[1], lrps[-1]
    forwd_pq, backwd_pq = [lrp for lrp in lrps[::-1]], lrps.copy()
    
    heapify(forwd_pq), heapify(backwd_pq)

    # track processed lrps with flag(0:forward, 1: backward) which search is used to initially process them
    ...
    while len(backwd_pq <= len(forwd_pq) and len(forwd_pq) >= 2 and len(backwd_pq) >= 2:
        ...
        # find candidates
        forwd_lrp_cand = list(nominate_candidates(forwd_lrp, reader, config, len(forwd_pq)==1))
        forwd_nxt_lrp_cand = list(nominate_candidates(forwd_nxt_lrp, reader, config, len(forwd_pq)==1))

        backwd_lrp_cand = list(nominate_candidates(backwd_lrp, reader, config, len(backwd_pq)==len(processed_lrps)))
        backwd_prev_lrp_cand = list(nominate_candidates(backwd_prev_lrp, reader, config, len(backwd_pq)==len(processed_lrps)))
        ...
        backwd_pairs.sort(key=lambda pair: (pair[0].score + pair[1].score), reverse=True)
        
        # mín and max path length to the next lrp
        forwd_minlen = (1 - config.max_dnp_deviation) * forwd_lrp.dnp - config.tolerated_dnp_dev
        forwd_maxlen = (1 + config.max_dnp_deviation) * forwd_lrp.dnp + config.tolerated_dnp_dev
        forwd_lfrc = config.tolerated_lfrc[forwd_lrp.lfrcnp]

        # mín and max path length to the next lrp
        backwd_minlen = (1 - config.max_dnp_deviation) * backwd_prev_lrp.dnp - config.tolerated_dnp_dev
        backwd_maxlen = (1 + config.max_dnp_deviation) * backwd_prev_lrp.dnp + config.tolerated_dnp_dev
        ...
        for idx in range(K, 0, -1):
            ...
            forwd_route = handleCandidatePair((forwd_lrp, forwd_nxt_lrp), forwd_pairs[idx], observer, forwd_lfrc, forwd_minlen, forwd_maxlen)
            backwd_route = handleCandidatePair((backwd_prev_lrp, backwd_lrp), backwd_pairs[idx], observer, backwd_lfrc, backwd_minlen, backwd_maxlen)
